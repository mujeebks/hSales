//TransportationReportController

'use strict';
MetronicApp.controller('TransportationReportController', ['$scope', 'dataService', '$filter', 'NotificationService', 'ApiUrl', 'commonService', '$controller', 'HelperService',
  
    function ($scope, dataService, $filter, NotificationService, ApiUrl, commonService, $controller, HelperService) {

        $scope.dayReport = "Day.csv";
        $scope.detailReport = "Detail.csv";
        $scope.ReportHeading = "Driver Trip Report";
        ...
        $scope.GetDriverDayAndDetailedReport=   function(DriverCode,DriverName)
        {
            Metronic.blockUI({ boxed: true });
            $scope.dayReport = "DRIVER_TRIP_BY_DAY_REPORT_" + DriverCode+".CSV";
            $scope.detailReport = "DRIVER_TRIP_BY_DETAIL_REPORT_"+ DriverCode+".CSV";
            $scope.DetailedDayReport = true;
            $scope.ReportheadAttachment = "(" + DriverCode + "  " + DriverName + ")";
            ...
            (httpRequest = dataService.GetDriverDayAndDetailedReport(param))
                .then(function (response) {
                    if (response) {
                        $scope.iscanceldisabled = true;
                        $scope.transportationdriverdetailcsv = "TRANSPORTATION-DRIVER_TRIP_DETAIL-REPORT.CSV";
        ...
        $scope.search = function (filter) {
            ...
            if (!status &&
                ...
                (httpRequest = dataService.GetTransportationDriverReport(param))
                    .then(function (response) {
                        if (response) {
                            $scope.iscanceldisabled = true;
                            $scope.transportationdrivercsv = "TRANSPORTATION-DRIVER-TRIP-REPORT.CSV";
                            $scope.transportationdrivercsv = $scope.transportationdrivercsv.toUpperCase();
                            $scope.DriverTripConsolidatedReportList = response.data;
                            $scope.DriverTripConsolidatedReportListCopy = angular.copy(response.data);
                            ...
                            var totalstop = 0;
                            var totalcases = 0;
                            for (var i = 0; i < $scope.DriverTripConsolidatedReportList.length; i++)
                            {
                                
                                TotalPrevious = TotalPrevious + $scope.DriverTripConsolidatedReportList[i].Previous;
                                TotalCurrent = TotalCurrent + $scope.DriverTripConsolidatedReportList[i].Current;
                                totaltrip = totaltrip + $scope.DriverTripConsolidatedReportList[i].NumberOfTrips;
                                totalstop = totalstop + $scope.DriverTripConsolidatedReportList[i].NumberOfStops;
                                totalcases = totalcases + Number.parseFloat( $scope.DriverTripConsolidatedReportList[i].CasesDelivered+'');
                            ...
                            $scope.TotalNoofTrips = $filter('numberWithCommasRounded')(totaltrip);
        ...
        $scope.resetSearch = function () {
          //  $scope.myTableFunctions.resetSearch();
            $scope.groupProperty = '';
            $scope.DriverTripConsolidatedReportList = [];
            $scope.DriverTripConsolidatedReportListCopy = [];
        ...
        $scope.getCsvHeader = function () {
            return ['Driver Code', 'Driver Name', 'Route Type','Number Of Trips','Number Of Stops','Cases Delivered'];
        }

        $scope.getCsvHeaderDay = function () {
            return [ 'Date','Route', 'Route Type',  'Number Of Stops', 'Cases Delivered'];
        }
        ...
        $scope.getCsvHeaderCustomer = function () {
            return [ 'Truck Code', 'Route', 'Route Type',  'Date',
        ...
        $scope.getCsvData = function () {
            ...
            predefinedHeader = ['DriverCode', 'DriverName', 'RouteType', 'NumberOfTrips', 'NumberOfStops', 'CasesDelivered'];

            angular.forEach($scope.DriverTripConsolidatedReportListCopy, function (value, key) {
                var header = {};
        ...
        $scope.getCsvDataDay = function () {
            ...
            predefinedHeader = [ 'InvoiceDateString', 'Route', 'RouteType', 'NumberOfStops', 'CasesDelivered'];
            ...
            total[4] = "";
            total[5] =  totalstops;
            total[6] = $filter('numberWithCommasRounded')(Number.parseInt(totalcases));
        ...
        $scope.getCsvDataDetailCustomer = function () {
            ...
            predefinedHeader = [ 'TruckCode', 'Route', 'RouteType',  'InvoiceDateString',
            ...
            total[11] = invoice;
            total[12] = $filter('numberWithCommasRounded')(Number.parseInt(totalcases));
